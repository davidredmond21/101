...
# a) Which of the following will find the log of the square root of the sequence 1
#to 10? (Tick all that apply)
1:10 %>% sqrt %>% log(base = exp(1)) ###this one
1:10 %>% `^`(0.5) %>% log ###this one
log(sqrt(1:10)) ####and this one
sqrt(log(1:10))
1:10 %>% log %>% sqrt
1:10 %>% sqrt %>% log #### this ONE

# b) The function below to takes 1000 uniform random variables, converts
# them to normal using qnorm, then produces a histogram.
# Re-write it in magrittr format (for Blackboard marking purposes
#keep everything on one line and use spaces between %>% and =, etc)
hist(qnorm(runif(1000)), breaks = 30)
runif(1000) %>% qnorm %>% hist(breaks = 30) #### this one
...
# - The within statement allows us to create a new variable (within is a
# base function) whilst keeping the whole data frame (different from with)
...
# Take the iris data (included by default in R) and write magrittr code that:
  iris %>% subset(Sepal.Length > 3) %>%
           aggregate( . ~ Species, data = ., FUN = 'median') %>%
           within(., Petal.Area <- pi %>% multiply_by(Petal.Length) %>%
           divide_by(2) %>% multiply_by(Petal.Width) %>% divide_by(2))
...
# Fill in the blanks so that the 2-column data set used to create
# the plot is returned as part of the call
# COMPLETE
mtcars %>% within(., log_mpg <- mpg %>% sqrt) %>% #[A]
   within(., log_wt <- wt %>% log) %>% # [B]
   extract(c('log_mpg', 'log_wt')) %T>% # [C]
   plot
...
# I want to create my own function that produces the mean, sd,
# and length for use in an aggregate call
# Write a magrittr defined function to return the mean, sd,
# and length of a vector
# It should work with

mtcars %>% aggregate(mpg ~ cyl, data = ., my_fun)
