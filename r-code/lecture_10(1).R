...
stan_model_lr = stan_model('stan_files/lin_reg_1.stan')
...
stan_model_lr_2 = stan_model('stan_files/lin_reg_2.stan')
...
stan_model_lr_3 = stan_model('r_code/lecture_10_files/lin_reg_3.stan')
...
## EXERCISE 1
# Using version 3 of the stan code, fit Bayesian models which have multiple explanatory variables. These extra explanatory variables should be powers of x (go back to lecture 2 e.g. lines 62 onwards to see how this is done). Fit a model with intercept, linear, square and cubic components. Look at the 50% intervals produced by the model. What are the 50% intervals (to 2dps) for each term
# Linear: [A]
# Quadratic: [B]
# Cubic: [C]
...
# See lin_reg_4.stan
stan_model_lr_4 = stan_model('r_code/lecture_10_files/lin_reg_4.stan')
...
## EXERCISE 2
# a) Returning to the simple linear regression model, edit the parameters block of the lin_reg_4.stan file to add an upper bound of 0.9 on sigma, and a lower and upper bound of 0 and 1 respectively on alpha. Run the model using the sampling function to produce full probability distributions on the parameters. What is the median value of beta (to 2 dp)?
# b) Edit the model block of the lin_reg_4.stan (removing any changes you made from part a) to produce the same constraints (hint: add in a uniform prior on sigma and alpha) What are your two extra lines in the model block? (hint 2: you can check this worked because you will get the same value of beta from part a)
...
stan_model_bin_reg = stan_model('r_code/lecture_10_files/bin_reg.stan')
stan_run_bin_reg = optimizing(stan_model_bin_reg,
                              data = stan_data_bin_reg)
...
earnings_model_lr = stan_model('r_code/lecture_10_files/lin_reg_1.stan')
...
earnings_data_lr_2 = list(N = nrow(earnings),
                          N_cat = 4,
                          x = earnings$height_cm,
                          y = earnings$y,
                          cat = earnings$eth)
...
earnings_model_lr_2 = stan_model('r_code/lecture_10_files/lin_reg_5.stan')
earnings_run_lr_bayes_2 = sampling(earnings_model_lr_2, data = earnings_data_lr_2)
plot(earnings_run_lr_bayes_2) # Now have a intercept and slope for each
plot(earnings_run_lr_bayes_2, pars = 'beta') # Now have a intercept and slope for each
...
earnings_model_lr_3 = stan_model('r_code/lecture_10_files/lin_reg_6.stan')
...
## EXERCISE 4
# Create a ggplot of the fitted lines from each group using the model in lin_reg_5.stan (adapted if required). Use a facet command to put each ethnic group in a different panel. (Extra kudos if you can get the 50% uncertainty bands on the plots too). Which ethnic group shows the weakest relationship between height and log earnings?
