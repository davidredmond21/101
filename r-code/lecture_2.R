...
# Here X and X_g are a design matrices appled to the covariate and grid values respectively
...
# 2. For an n by m matrix X, how many operations is t(X) %*% X?
# 3. If n is much bigger than m, which has fewer flops t(X)%*%X or X%*%t(X)
...
for(i in 1:grid_size) sig_ll[i] = GP_ll(c(sig_sq_grid[i], 10, 2),x)
...
for(i in 1:length(sig_ll)) sig_ll[i] = GP_ll(c(both_grid[i,1], 2, both_grid[i,2]),x)
...
all_grid = expand.grid(sig_sq_grid, rho_sq_grid, tau_sq_grid)
...
all_out[which(all_out[,4]==min(all_out[,4])),]
# 787
#
# 2 What are the values of sig_sq, tau_sq and rho_sq respectively that
#minimise the log-likelihood (to 3 d.p. - separate answers with a space)?
# 1.647 5 0.358
#
...
# which f(theta^new) should be lower. This information might concern the first and second
# derivatives of f if available, or anything else to hand. Many optimisation algorithms only
# update one element of theta at a time, but with more information it's usually more efficient to
# update multiple elements
# 3) If f(theta^new) is really similar to f(theta) then stop, otherwise repeat 2 again
# Perhaps the simplest (or most intuitive) way of optimising is to use a Taylor expansion of
# f(theta) and cut off at either one or two derivatives. If you cut off at 1 derivative then you
# are doing 'steepest descent' (sometimes called gradient descent), if you cut off at 2 you are doing Newton-Raphson
...
# In the previous exercise we created profile likelihoods by varying one parameter and
# optimising the other two over a grid of values. We're now in a position to write some code
# which, for every value on a grid for one parameter, optimises the remaining parameters.

# Write some code (using optim or nlminb) which for the following grid:
tau_sq_grid = seq(0.1, 2, length = 500)
answer_vec<-matrix(0,nrow=500,ncol=4)
# Standardise the data first
x = scale(motor$times)[,1]
y = scale(motor$accel)[,1]
# function GP_ll3 contains only 2 of the parameters to be varied with tua_sq fixed
GP_ll3 = function(p) {
  sig_sq = exp(p[1])
  rho_sq = exp(p[2])
  #tau_sq = exp(p[3])
...
#answer_vec<-list()
for (i in 1: length(tau_sq_grid)){
  tau_sq<<-tau_sq_grid[i]
  model3<-optim(c(0,0),GP_ll3,method='BFGS' )
  answer_vec[i,]<-cbind(model3$par[1],model3$par[2],model3$value,tau_sq)
}
#
ll_min<-min(answer_vec[,3])

opt<-answer_vec[which(answer_vec[,3]==ll_min),]
cat("optimum", opt[4], " \n")

exp(opt[4]) ##  1.206321e+00
## this means tau_sq should be 1.206
opt[3]
## which gives a best score of log-likeli == 72.11
plot(answer_vec[,4],answer_vec[,3], ylab="-ll", xlab="tau_sq")
...
GP_ll3(c(sig_sq,rho_sq))

plot(tau_sq_grid,answer_vec)
exp(tau_sq_grid[which(answer_vec== min(answer_vec))])


# a) optimises the values of sig_sq and rho_sq for every value on the grid
#
# b) plots the tau_sq_grid values against the log likelihood
# Hint 1: you'll need to re-write the GP_ll2 (or GP_ll) function to hold the tau_sq
# values constant
...
#
# It's very important to be able to generate good uniform random numbers for all kinds
...
# If you're now performing a stochastic method which requires equal coverage
# in high dimensions you're going to run into problems.
...
# One thing that's often interesting to look for is how often certain orderings occur,
# e.g. u[i-1] < u[i+1] < u[i]. This order should happen about one sixth of the time
#
...
# Using the seeds x[1] = x[2] = 1 and M as before (in the Marsaglia sequence), generate a large set of values from this sequence
