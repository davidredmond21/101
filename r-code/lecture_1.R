...
# 1) Create a cool plot of the prostate data exploring the use of colours,
# labels. See if you can find something interesting about it that
#  otherwise might not be obvious. Try to use some commands that
#  haven't been used elsewhere, e.g. polygon, image, etc.
...
#
prostate = read.table('http://statweb.stanford.edu/~tibs/ElemStatLearn/datasets/prostate.data', header = TRUE)
range = quantile(prostate$age)
#
nf<-layout(matrix(c(2,0,1,3),2,2,byrow=TRUE),c(3,1),c(1,3),TRUE)
layout.show(nf)
pro_train<-prostate[which(prostate$train==TRUE),]
pro_test<-prostate[which(prostate$train==FALSE),]
xhist<-hist(prostate$age,40,plot=FALSE)
yhist<-hist(prostate$lweight,40,plot=FALSE)
top<-max(xhist$counts,yhist$counts)
model<-lm(pro_train$lcavol~pro_train$lpsa)
par(mar=c(2,0,1,1))
plot(pro_train$lcavol,pro_train$lpsa,
     xlab="lcavol",ylab="lpsa",col= "green",pch=19)
points(pro_test$lcavol,pro_test$lpsa, col= "red", pch=19)
lines(model<-glm(prostate$lcavol~prostate$lpsa)$fitted.values,
      prostate$lpsa, col = rgb(.9, .1, 0, alpha = 0.2),lwd='4')
par(mar=c(0,3,1,1))
barplot(xhist$counts,axes=FALSE,col="green",space=0,ylim=c(0,top))
par(mar=c(3,0,1,1))
barplot(yhist$counts,axes=FALSE,col="blue",space=0,xlim=c(0,top),horiz=TRUE)
legend( "topright", legend = c('training', 'test', "regression line"), pch=c(16,16,16),
       col = c('green', 'red', rgb(.9, .1, 0, alpha = 0.2)))


#
nf<-layout(1,1,space=0.4,axes=TRUE)#=TRUE)
layout.show(nf)
...
df4<-with(prostate,prostate[age>=range[4],])
df3<-with(prostate,prostate[age>=range[3] & age<range[4],])
df2<-with(prostate,prostate[age>=rbange[2] & age<range[3],])
df1<-with(prostate,prostate[age<range[2],])

model4<-loess(prostate$lcavol~prostate$lpsa+prostate$lcp,data=df4)
model3<-loess(prostate$lcavol~prostate$lpsa+prostate$lcp,data=df3)
model2<-loess(prostate$lcavol~prostate$lpsa+prostate$lcp,data=df2)
model1<-loess(prostate$lcavol~prostate$lpsa+prostate$lcp,data=df1)
xv<-seq(from= -0.1, to= 3.7, by=(3.8/length(prostate[,1]) ))
yv4<-predict(model4$y,xv,se=TRUE)
yv3<-predict(model3,xv)
yv2<-predict(model2,xv)
yv1<-predict(model1,xv)
par(mfrow=c(2,2))
plot(lcavol~lcp+lpsa,data=df4)
points(xv,yv4,col='red',pch=12)
plot(lcavol~lpsa+lcp,data=df3)
points(xv,yv3,col='green',pch=14)
plot(lcavol~lpsa+lcp,data=df2)
points(xv,yv2,col='blue',pch=16)
plot(lcavol~lpsa+lcp,data=df1)
points(xv,yv1,col='purple',pch=18)


points(prostate$lcavol~prostate$lpsa,subset=!prostate$train)
model1<-loess(prostate$lcavol~prostate$lpsa,subset=!prostate$train)
yt<-predict(model,xv)
lines(xv,yt,col='blue')

points(prostate$age>69, col="green")
...
x<-which[z$range]
top<-max(xhist$counts,yhist$counts)
plot(x$lcavol,x$lpsa,main="basic chart")
model<-lm(lcavol~lweight+lbph+svi+lcp+gleason+lpsa+pgg45,data=prostate)

w1<-prostate[x$age<range[2]&x$train==TRUE,]
w2<-prostate[x$age>=range[2]&x$age<range[3]&x$train==TRUE,]
w3<-prostate[x$age>=range[3]&x$age<range[4]&x$train==TRUE,]
w4<-prostate[x$age>=range[4]&x$train==TRUE,]
model1<-lm(lcavol~lweight+lbph+svi+lcp+gleason+lpsa+pgg45,data=w1)
model2<-lm(lcavol~lweight+lbph+svi+lcp+gleason+lpsa+pgg45,data=w2)
model3<-lm(lcavol~lweight+lbph+svi+lcp+gleason+lpsa+pgg45,data=w3)
model4<-lm(lcavol~lweight+lbph+svi+lcp+gleason+lpsa+pgg45,data=w4)
par(mfrow=c(2,2))
test<-prostate[x$age<range[2]&x$train==FALSE,]
test1<-test[,-1]
plot(w1$lcavol,w1$lpsa,main="age<60")



lines(model1$)
boxplot(model$residuals,model1$residuals, model2$residuals ,model3$residuals, model4$residuals)
par(mfrow=c(1,1))
