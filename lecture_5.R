...
# You might think: why am I not using subset? Well filter is neater because it 
# takes as many arguments as you like and joins them together with &

# You can do the same thing as subset though by combining boolean operators
flights %>% filter(month == 1 & day == 1 & carrier == 'B6')
# or, uglier
library(magrittr)
flights %>% filter(and(dep_time %>% equals(600), and(day %>% equals(1), 
                                                carrier %>% equals('B6'))))
...
slice(flights, 1)
slice(flights, 5:10)
slice(flights, nrow(flights)) # Gets the last row samew as tail(filghts)
...
# Again, you might think 'why not use order?' but of course that would 
# require lots more typing!

## EXERCISE 1  INCOMPLETE
# By default only the first few columns and rows of a tibble are printed. 
 #There is an options command to show all of the columns. What is it? 
# (Hint: you'll need to do some Googling to find this)
##  
 options(tibble.width = Inf) 
 flights[1:5,]
 options(tibble.width = 58) 
 
# Write a magrittr command that selects only January flights, 
 # then sorts the flights data by origin (ascending) and 
 # distance (descending), and finally glimpses the results
 
 flights %>% filter(month == 1) %>% arrange(desc(distance)) %>% arrange(origin) %>% glimpse
 
 ###test case
 mtcars  %>% as_tibble %>% arrange(desc(wt)) %>% arrange(mpg )  %>% filter(cyl == 6) %>% glimpse
...
flights %>% select(distance:arr_delay) #  From distance TO arr_del

mtcars %>% select(disp:wt) 

mtcar %>% cor(mtcars$disp, mtcars$wt)

mtcars %>%  select(disp,wt) %>% cor(.,use="complete.obs")
...
# Remember: filter for selecting rows, 
...
# One nice thing about mutate is that you can refer to already created variables
...
flights %>%  distinct(dest) %>% count
# destinations
flights %>%  distinct(dest,origin) %>% count
# destination/origin combinations
flights %>%  distinct(dest,origin,carrier) %>% count
...
# Write a magrittr format command that, for flights originating from JFK, 
# creates the average speed in miles per hour as:
...
flights %>% sample_frac(0.0001)

# Use replace = TRUE for a bootstrap sample
flights %>% sample_frac(0.00001, replace = TRUE)

# These functions by themselves are pretty useless. They 
# come into their own when used with group_by to group the data into groups
...
ggplot(out_tbl, aes(dist, delay)) +
  geom_point(aes(size = count), alpha = 1/2) +  # alpha makes them slight see-throu
  geom_smooth() +
  scale_size_area()  # scales the points and legend
# How cool is that!

# The summarise function now comes into its own,
# because we get something similar (and much more intuitive) than the aggregate 
# function, in far fewer lines of code

## EXERCISE 3
INCOMPLETE###################
# Use suitable group_by, summarise, and other dplyr/magrittr commands to answer the 
# following questions:

# Which plane (i.e. the tail number) spent the most amount of time in the air?
#
flights %>% group_by(tailnum) %>% summarise(group_by, max_time=max(na.rm = TRUE))
#
flights %>% summarise(tailnum,max_time=max(na.rm=TRUE))

# Use magrittr notation to do this in order
flights %>% summarize(max(air_time, na.rm = TRUE))

flights[which(flights$air_time==max(flights$air_time,na.rm=TRUE)),flights$tailnum]
# mean distancence
